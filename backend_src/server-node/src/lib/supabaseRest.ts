...
const getTableMetadata = async (table: string) => {
...
export async function selectRows(path: string) {
  if (usePostgres) {
    return selectRowsFromPostgres(path);
  }
...
export async function insertRow(path: string, payload: unknown) {
  if (usePostgres) {
    return insertRowInPostgres(path, payload);
  }
  return request(path, {
    method: "POST",
...
export async function updateRow(path: string, payload: unknown) {
  if (usePostgres) {
    return updateRowInPostgres(path, payload);
  }
  return request(path, {
    method: "PATCH",
    headers: { Prefer: "return=representation" },
...
export async function deleteRow(path: string) {
  if (usePostgres) {
    return deleteRowInPostgres(path);
  }
